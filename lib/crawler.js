...
class Crawler {
  ...
  _handlePageEvents() {
    ...
    this._page.setRequestInterception(this._options.setRequestInterception);
    this._page.on("request", (request) => void this._onPageRequest(request));
  }
  ...
  async _onPageRequest(request) {
    if (!this._options.onPageRequest) return;
    await this._options.onPageRequest(request, this._page, this._options);
  }
  ...
  async _collectLinks(baseUrl) {
    const links = await this._page.evaluate(() => {
      ...
      const findLinks = (document) => {
        document.querySelectorAll("a[href]").forEach((link) => {
          // @ts-ignore
          _links.push(link.href);
        });
        document.querySelectorAll("iframe,frame").forEach((frame) => {
          try {
            ...
            // @ts-ignore
            if (frame.src) _links.push(frame.src);
          }
        });
      };
    ...
    links.forEach((link) => {
      ...
      if (_link) mappedLinks.push(_link);
    });
    return uniq(mappedLinks);
  }
